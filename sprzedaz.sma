...
public print_annonce()
{
	...
	client_print_color(0, print_team_red, "^x04Chcesz sprzedac swoja bron? Wpisz^x03 /sprzedaj")
...
public cmd_sell(id)
{
	...
	{
		client_print_color(id, id, "^x04[SPRZEDAZ]^x01 Musisz byc w BuyZone, by sprzedac bron!")
		return PLUGIN_HANDLED
	}
	
	if(!is_user_alive(id))
	{
		client_print_color(id, id, "^x04[SPRZEDAZ]^x01 Musisz byc zywy, by sprzedac bron!")
		return PLUGIN_HANDLED
	}
	
	if(g_Round < 2)
	{
		client_print_color(id, id, "^x04[SPRZEDAZ]^x01 Sprzedaz broni w pierwszej rundzie jest zabroniona!")
		return PLUGIN_HANDLED
	}
	...
	if(price == 0)
	{
		client_print_color(id, id, "^x04[SPRZEDAZ]^x01 Nie mozesz tego sprzedac!")
		return PLUGIN_HANDLED
	}
	...
	client_print_color(id, id, "^x04[SPRZEDAZ]^x01 Dostales^x04 %d$^x01 za sprzedanie^x04 %s^x01!", cs_get_user_money(id) - oldmoney, weaponname[7])
	return PLUGIN_HANDLED
}
