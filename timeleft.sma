...
public plugin_init()
{
	...
	register_clcmd("say tl", "sayTimeLeft", 0, "- displays timeleft")
	register_clcmd("say tt", "sayTheTime", 0, "- displays current time")
...
public sayTheTime(id)
{
	...
	get_time("^4%m^1/^4%d^1/^4%Y^1 - ^4%H^1:^4%M^1:^4%S^1", ctime, charsmax(ctime))
	client_print_color(0, 0, "%L: %s.", LANG_PLAYER, "THE_TIME", ctime)
...
public sayTimeLeft(id)
{
	if (get_pcvar_float(g_mp_timelimit))
	{
		...
		client_print_color(0, 0, "%L: ^x04%d^x01:^x04%02d^x01.", LANG_PLAYER, "TIME_LEFT", (a / 60), (a % 60))
	}
	else
		client_print_color(0, 0, "%L.", LANG_PLAYER, "NO_T_LIMIT")
