...
new Maps[Max][32], LastMaps[Max][32], MapName[32], MapNames[MaxChoice][32], MapRevote[2][32], AllMaps[Max][32], NomMaps[Max][32], NomNum, Nom, Nomed[Max];
...
public plugin_init()
{
	...
	Cvar_ChangeSpeed = register_cvar("umc_changespeed", "3.0");
	...
	Cvar_MenuDestroyTime = register_cvar("umc_menudestroytime", "1.0");
	...
	Cvar_MaxMaps = register_cvar("umc_maxmaps", "5");
	...
	Cvar_AllowExtend = register_cvar("umc_allowextend", "0");
	...
	Cvar_StartButton = register_cvar("umc_startbutton", "5");
...
public plugin_cfg()
{
	set_task(0.25, "LoadAllMaps", TASK_LOAD);

	set_task(0.1, "SavePresentTime");
}
...
public RestartRound() 
{
	if(get_pcvar_num(Cvar_Mod) == 1)
	{
		if(TimeLimit > 0) server_cmd("mp_timelimit %d", TimeLimit);
		else server_cmd("mp_timelimit 20");
	}
	else
	{
	...
	for(Num = 0; Num < sizeof(NomMaps); Num++)
		NomMaps[Num] = "";
	
	for(Num = 0; Num < sizeof(Nomed); Num++)
		Nomed[Num] = 0;
	
	for(Num = 0; Num < sizeof(MapNames); Num++)
	{
		MapNames[Num] = "";
		MapVote[Num] = 0;
	}
...
public NomMap(id, szMap[32])
{
	...
	if(Already == 1 || !isValidMap(szMap, 1) || Nomed[Nom] == 1)
	{
		...
		if(Time[0] <= 3 && Time[1] == 0 && Time[2] == 0)
		{
			...
			if(!ChangeMap && Voted && !inProcess && !Revoted || !ChangeMap && Revoted && !inProcess && !Voted)
			{
				if(get_pcvar_num(Cvar_OnlyNextRound) == 1)
				{
					...
					ChangeMap = true;
					log_amx("[MAP] Mapchange on next round: %s", Next);
				}
...
public Counter()
{
	if(MapCounter < 1)
	{
		...
		for(new Num = 1; Num <= MAX_PLAYERS; Num++)
		{
			if(!is_user_connected(Num) || is_user_hltv(Num))
				continue;
...
{	
	...
	for(new Num; Num < get_pcvar_num(Cvar_MaxMaps); Num++)
	{
		...
		if(equal(MapName, MapNames[Num]))
			formatex(String, 127, "%L", Lang, "MOP5", Num+StartButton == 10 ? 0 : Num+StartButton, MapNames[Num], MapVote[Num], MapPercent[Num]);
		else
			formatex(String, 127, "%L", Lang, "MOPD", Num+StartButton == 10 ? 0 : Num+StartButton, MapNames[Num], MapVote[Num], MapPercent[Num]);
...
public VoteMenuKeys(id, Key)
{
	...
	if(PlayerVoted[id] || Voted || Key < 0)
	{
		...
		if(PlayerVoted[id] && Key > -1) print_color(id, "%s %L", Prefix, Lang, "AVO");
		
		set_task(0.25, "ShowVoteMenu", id);
...
public LoadVoteMaps()
{
	new Line[128], Map[32], Len, Used[Max], File, Found, Loaded = 0;
	...
	if(Loaded > 0)
	{
		log_amx("[MAP] Loaded %i maps for vote.", Loaded);
	...
	else set_task(0.25, "LoadVoteMaps", TASK_LOAD);
}

public LoadNomMaps()
{
	new Line[128], Map[32], File, Found, NomNum = 0;
	...
	log_amx("[MAP] Loaded %i maps for nomination.", NomNum);
...
public LoadAllMaps()
{
	new Line[128], Map[32], File, MapNum = 0;
	...
	set_task(0.1, "LoadLastMaps", TASK_LOAD);
}
...
public LoadMapMenu()
{
	...
	set_task(0.1, "LoadVoteMaps", TASK_LOAD);
}
...
public LoadLastMaps()
{
	...
	set_task(0.1, "LoadMaps", TASK_LOAD);
}
...
public LoadMaps()
{
	...
	new Line[64], File, MapDatas[2][32], LineNum, MapNum = 0, bool:Found;
	File = fopen(MapFile, "rt");
	...
	set_task(0.1, "LoadMapMenu", TASK_LOAD);
}
...
public RevoteMenuKeys(id, Key)
{
	...
	if(PlayerRevoted[id] || Revoted || Key < 0)
	{
		...
		if(PlayerRevoted[id] && Key > -1) print_color(id, "%s %L", Prefix, Lang, "AVO");

		set_task(0.25, "ShowRevoteMenu", id);
...
stock CheckVotes() 
{
	if(AllVotes == 0)
	{
		...
		set_cvar_string("amx_nextmap", Next);
		NextMap();
		log_amx("[MAP] No Votes. Next map: %s", Next);
		return PLUGIN_HANDLED;
	}
	...
	//if(!strlen(MapNames[VoteNum_1])) MapNames[VoteNum_1] = MapNames[0];
...
stock CheckRevotes() 
{
	if(AllRevotes == 0)
	{
		Next = MapRevote[0];
		set_cvar_string("amx_nextmap", Next);
		log_amx("[MAP] Revote. No Votes. Next map: %s", Next);
	...
	//if(!strlen(MapRevote[VoteNum_1])) MapRevote[VoteNum_1] = MapNames[0];
	
	Next = MapRevote[VoteNum_1];
	set_cvar_string("amx_nextmap", Next);
	log_amx("[MAP] Revote. Next map: %s", Next);
...
stock bool:isValidMap(szMap[], iNom = 0)
{
	...
	if(iNom)
	{
		for(new MapNum; MapNum < MapCycle_Lines; MapNum++)
		{
			if(equali(szMap, AllMaps[MapNum]) && is_map_valid(szMap))
				return true;
		}
	}
	else
...
stock ChangeWeapon(id, slot)
{
	...
	switch(slot)
	{
		case 1: { engclient_cmd(id, "slot1"); client_cmd(id, "slot1"); }
		case 2: { engclient_cmd(id, "slot2"); client_cmd(id, "slot2"); }
		case 3: { engclient_cmd(id, "slot3"); client_cmd(id, "slot3"); }
		case 4: { engclient_cmd(id, "slot4"); client_cmd(id, "slot4"); }
		case 5: { engclient_cmd(id, "slot5"); client_cmd(id, "slot5"); }
	}
}
