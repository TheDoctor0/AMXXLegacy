...
USAGE
=====
Bez komand itp.. Obstawia siê za pomoc¹ menu, które "wyskakuje"...

S¹ trzy cvary:
- lastmanbets_overridemenus
Normalnie ustawione jest na: 1. Mo¿esz ustawiæ to na 0, wtedy osoby z innym menu otwartym (amxmodmenu, wc3 menu, etc) nie zobacz¹ okna obstawiania. Notatka, od wersji 0.9 jest pewien bug z detekcj¹ menu.

- lastmanbets_defaultbet
Normalnie ustawione jest na: 100. Jest to minimalna liczba kast ($) jak¹ mo¿na postawiæ.

- lastmanbets_bettime
Normalnie ustawione jest na: 20 (sekund). Cvar ten ustawia, jak d³ugo menu bêdzie otwarte - 'stoper' jest u do³u ekranu, wyœrodkowany. Je¿eli dojdzie on do 0, wszystkim, co mieli to menu otwarte, zostanie zamkniête.
...
public death_event() {
	...
	if (ctsnum != 1 || tsnum != 1 || !jail_is_fight()) {
...
displaybetshud() {
	...
	format(tMessage, MESSAGESIZE, "%s^n$%d z %d zakladow", g_names[TEAM_T], betamounts[0], betcount[0])
	format(ctMessage, MESSAGESIZE, "%s^n$%d z %d zakladow", g_names[TEAM_CT], betamounts[1], betcount[1])
...
public respawn(idd[1]) {
	...
	client_print(0, print_chat, "%s respawned, posiada jeszce %d zyc!", name2, g_extralives[idd[0]], g_extralives[idd[0]] == 1 ? "fe" : "ves")
}
...
prize(id, moneyover16k) {
	...
	//client_print(id, print_chat, "Jezeli wygrasz $%d okolo $16000, wygrasz takze te nagrody: %d dodatkowe zycia, %d zwiekszenie zdrowania, %d dodatkowy atak i tym podobne")
}
...
setprizes(id) {
	...
	//	client_print(id, print_chat, "Do wygarnia: %d dodatkowe zycia, %d zwiekszenie zdrowia, %d dodatkowy atak", g_extralives[id], g_extrahealth[id], extrahealthused ? " (1 just used)" : "", g_extraarmour[id], extraarmourused ? " (1 just used)" : "")
}
...
public giveprize(id, level, cid) {
	...
	//console_print(id, "Dalismy %d przedmiotow typu %d do %d.", prizes, theprize, giveto)
...
public hudtimerdisplay(timeend[1]) {
	...
	if (secondsleft > 0) {
		msg = "Menu obstawiania zamknie sie za %d sekund"
		format(msg, 63, msg, secondsleft)
	}
	else
		msg = "Nie obstawiles wystarczajaco szybko..."
...
startbet(id) {
	...
	new len = format(menuBody, 511, "Pojedynek \y%s\w vs. \y%s\w! Obstawiajce! Kto wygra?^n^n1. %s (HP: %d, Armor: %d)^n2. %s (HP: %d, Armor: %d)^n^nAktualne stawki: $\y%d\w^n", g_names[TEAM_T], g_names[TEAM_CT], g_names[TEAM_T], t_hp, t_armour, g_names[TEAM_CT], ct_hp, ct_armour, g_betamount[id])
	...
	len += format(menuBody[len], 511 - len, "3. Podnies stawke o $%d^n", DEFAULTBET)
	...
	if (money >= g_betamount[id] + DEFAULTBET * 10) {
		...
		len += format(menuBody[len], 511 - len, "\d")
	len += format(menuBody[len], 511 - len, "4. Podnies stawke o $%d^n", DEFAULTBET * 10)
	...
	if (g_betamount[id] - DEFAULTBET >= DEFAULTBET) {
		...
		len += format(menuBody[len], 511 - len, "\d")
	len += format(menuBody[len], 511 - len, "5. Obniz stawke o $%d^n", DEFAULTBET)
	...
	if (g_betamount[id] - DEFAULTBET * 10 >= DEFAULTBET) {
		...
		len += format(menuBody[len], 511 - len, "\d")
	len += format(menuBody[len], 511 - len, "6. Obniz stawke o $%d^n\w", DEFAULTBET * 10)

	len += format(menuBody[len], 511 - len, "7. Postaw wszystko!")
	flags |= MENUBUTTON7

	len += format(menuBody[len], 511 - len, "^n^n0. Nie jestem hazardzista")
...
public menu_fn(id, key) {
	...
	switch (key) {
		...
		case MENUSELECT0: {
			// no bet
			if (g_betting)
				client_print(id, print_chat, "Dobrze, nie obstawiaj w ogole. Zachowaj pieniadze!")
			stayinmenu = false
		}
	}
...
public closebetmenu() {
	...
	for (new i = 1; i <= MAXPLAYERS; i++) {
		...
		if (usermenu == g_betmenu) // Hide it here!
			client_cmd(i, "slot10") // client_print(players[i], print_chat, "Hey, runda sie zakonczyla, a Ty w ogole nie obstawiles... zamknij to menu!")
	}
}
...
payback() {
	...
	for (new i = 1; i < 33; i++) {
		...
		client_print(i, print_chat, "Runda zakonczyla sie remisem. Oddajemy Ci $%d.", g_betamount[i])
	}
}
...
calculatebets(result) {
	...
	if (overallbets == 0) {
		client_print(0, print_chat, "Nikt nie obstawial w tej rundzie...")
		return
	}

	// Print total bets.
	client_print(0, print_chat, "Cala pula: $%d ($%d na zwycieska druzyne)", overallbets, totalrightbets)
	...
	for (new i = 1; i < 33; i++) {
		...
		if (totalrightbets > 0 && g_betperson[i] == result) { // Just to avoid divison by 0 (which shouldn't happen, but...)
			...
			client_print(i, print_chat, "Wygrales! Zwracamy Ci Twoje pieniadze, $%d, wraz z wygarna $%d!", g_betamount[i], floatround(wonmoney) - g_betamount[i])
			server_print("%s wygral i dostaje swoje pieniadze, $%d, wraz z wygrana $%d!", name, g_betamount[i], floatround(wonmoney) - g_betamount[i])
			...
			client_print(i, print_chat, "Nie obstawiles tym razem.")
	...
	if (nr_of_highestwinners == 0) { // Did anyone make the right bet? If not, move the bet money to the pot.
		client_print(0, print_chat, "Nikt nie wygral. $%d jest w puli. Razem: $%d", overallbets, g_pot += overallbets)
		set_vaultpot(g_pot)
		return
	}
		...
		if (g_pot != 0) {
			client_print(0, print_chat, "%s wygral $%d ze stawka $%d, wygral takze pozostala pule, $%d!", name, floatround(highestwonmoney), g_betamount[highestwinners[0]], g_pot)
			give_pot(nr_of_highestwinners, highestwinners)
		}
		else {
			if (highestwonmoney >= 1.0)
				client_print(0, print_chat, "%s wygral $%d ze stawka $%d, pula nagrod jest pusta.", name, floatround(highestwonmoney), g_betamount[highestwinners[0]])
		}
	}
		...
		if (g_pot > 0) {
			...
			len += format(msg[len], 127 - len, "Najlepsi zwyciezcy: ")
...
give_pot(nr_of_winners, winners[32]) {
	...
	if (nr_of_winners == 0) {
		log_amx("%s: Blad skryptu - division by zero error in give_pot! nr_of_winners: %d", nr_of_winners)
		return
	}
...
#endif
#endif
	//register_clcmd("0killtest", "killtest", ADMIN_CFG, "- zabil Cie i pozostaly dwie osoby zywe w kazdej druzynie...")
	register_cvar(OVERRIDEPREVIOUSMENU, "0")
