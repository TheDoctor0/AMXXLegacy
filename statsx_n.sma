...
format_top15(sBuffer[MAX_BUFFER_LENGTH + 1])
{
	...
	{
		get_stats_sql(i, izStats, izBody, t_sName, MAX_NAME_LENGTH)
...
format_rankstats(id, sBuffer[MAX_BUFFER_LENGTH + 1], iMyId = 0)
{
	...
	iRankPos = get_user_stats_sql(id, izStats, izBody)
	iLen = format(sBuffer, MAX_BUFFER_LENGTH, "<body bgcolor=#000000><font color=#FFB000><pre>")
	iLen += format(sBuffer[iLen], MAX_BUFFER_LENGTH - iLen, "%L %L^n^n", id, (!iMyId || iMyId == id) ? "YOUR" : "PLAYERS", id, "RANK_IS", iRankPos, get_statsnum_sql())
...
public eventShowRank(id)
{
	if (SpecRankInfo && g_izSpecMode[id])
	{
		...
		if (is_user_connected(iPlayer))
		{
			...
			iRankPos = get_user_stats_sql(iPlayer, izStats, izBody)
			iRankMax = get_statsnum_sql()
