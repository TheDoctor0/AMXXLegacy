...
#define VERSION "1.0"
#define AUTHOR "serfreeman1337 & O'Zone"
...
enum _:player_data_struct
{
	...
	PLAYER_STEAMID[30],
	PLAYER_IP[16],
	PLAYER_STATS3[STATS3_END],
	PLAYER_STATS3LAST[STATS3_END],
	PLAYER_FIRSTJOIN,
	PLAYER_LASTJOIN
}

enum _:stats_cache_struct
{
	CACHE_NAME[32],
	CACHE_STEAMID[30],
	CACHE_STATS[8],
...
public plugin_precache()
{
	...
	cvar[CVAR_SQL_HOST] = register_cvar("csstats_sql_host","sql.pukawka.pl",FCVAR_UNLOGGED|FCVAR_PROTECTED)
	cvar[CVAR_SQL_USER] = register_cvar("csstats_sql_user","510128",FCVAR_UNLOGGED|FCVAR_PROTECTED)
	cvar[CVAR_SQL_PASS] = register_cvar("csstats_sql_pass","xvQ5CusRVCVzj83aruWk",FCVAR_UNLOGGED|FCVAR_PROTECTED)
	cvar[CVAR_SQL_DB] = register_cvar("csstats_sql_db","510128_stats",FCVAR_UNLOGGED|FCVAR_PROTECTED)
	...
	if(!cvar[CVAR_RANK]) cvar[CVAR_RANK] = register_cvar("csstats_rank","1")
	cvar[CVAR_RANKBOTS] = get_cvar_pointer("csstats_rankbots")
	if(!cvar[CVAR_RANKBOTS]) cvar[CVAR_RANKBOTS] = register_cvar("csstats_rankbots","0")
	...
	cvar[CVAR_ASSISTHP] = register_cvar("csstats_sql_assisthp","60")
}
...
public plugin_cfg()
{
	...
	server_cmd("exec %s/csstats.cfg", configPath);
	server_exec();
...
public EventHook_Damage(player)
{
	...
	if(!pev_valid(dmg_inflictor))
	{
		return PLUGIN_CONTINUE
	}

	new weapon_id,last_hit,attacker

	attacker = get_user_attacker(player,weapon_id,last_hit)
	
	if(!(0 < dmg_inflictor <= MAX_PLAYERS))
	{
...
Stats_SaveKill(killer,victim,wpn_id,hit_place)
{
	...
	if(!is_tk(killer, victim))
	{
		...
		static name[32], name2[32], weapon[32];

		get_user_name(killer, name, charsmax(name));
		get_user_name(victim, name2, charsmax(name2));
		get_weaponname(wpn_id, weapon, charsmax(weapon));
	}
	else
	{
...
DB_LoadPlayerData(id)
{
	...
	//log_to_file("csstats.log", player_data[id][PLAYER_NAME])
...
DB_SavePlayerData(id,bool:reload = false)
{
	...
	{
		//log_to_file("csstats.log", "Not loaded %s", player_data[id][PLAYER_NAME]);
		return false
	}
	...
	switch(player_data[id][PLAYER_LOADSTATE])
	{
		...
		{
			//log_to_file("csstats.log", "Save on kill %s", player_data[id][PLAYER_NAME]);
...
public SQL_Handler(failstate,Handle:sqlQue,err[],errNum,data[],dataSize){
	...
	switch(data[0])
	{
		...
		case SQL_LOAD: // загрзука статистики игрока
		{
			...
			{
				//log_to_file("csstats.log", "Disconnected? %s", player_data[id][PLAYER_NAME])
				return PLUGIN_HANDLED
			}
			...
			if(SQL_NumResults(sqlQue)) // считываем статистику
			{
				...
				//log_to_file("csstats.log", "Data Loaded %s", player_data[id][PLAYER_NAME]);
			}
...
public plugin_natives()
{
	...
	register_native("get_wstats_sql_thread","native_get_wstats_thread")
...
public native_get_wstats_thread(plugin_id,params)
{
	if(!is_ready)
	{
		return false
	...
	new szTemp[1024], Data[2];
	Data[0] = id;
	Data[1] = weapon;
	
	if(weapon == CSW_KNIFE) format(szTemp, charsmax(szTemp), "SELECT a.name, b.kills, b.hs FROM `%s` a JOIN `%s_weapons` b on a.id = b.player_id WHERE b.weapon = '%s' ORDER BY kills DESC, hs DESC LIMIT 15", tbl_name, tbl_name, weaponname);
	else if(weapon == CSW_HEGRENADE) format(szTemp, charsmax(szTemp), "SELECT a.name, b.kills FROM `%s` a JOIN `%s_weapons` b on a.id = b.player_id WHERE b.weapon = '%s' ORDER BY kills DESC LIMIT 15", tbl_name, tbl_name, weaponname);
	else format(szTemp, charsmax(szTemp), "SELECT a.name, b.kills, b.deaths, b.hs, b.shots, b.hits, (SELECT COUNT(*) FROM `%s_weapons` WHERE (kills-deaths)>=(b.kills-b.deaths) AND weapon = '%s') as rank FROM `%s` a JOIN `%s_weapons` b on a.id = b.player_id WHERE b.weapon = '%s' ORDER BY rank LIMIT 15", tbl_name, weaponname, tbl_name, tbl_name, weaponname);
	
	SQL_ThreadQuery(sql, "native_get_wstats_thread_handle", szTemp, Data, 2);
	
	return true;
}

public native_get_wstats_thread_handle(FailState, Handle:Query, Error[], ErrCode, Data[], DataSize)
{
	if(FailState) 
	{
	...
	static szName[32], iKills, iDeaths, iHS, iShots, iHits, iRank;

	if(Data[1] == CSW_KNIFE)
	{
		iLen = format(sBuffer, 2047, "<body bgcolor=#000000><font color=#FFB000><pre>")
		iLen += format(sBuffer[iLen], 2047 - iLen, "%2s %-22.22s %10s %13s^n", "#", "Nick", lKills, "HS")
		...
		while(SQL_MoreResults(Query))
		{
			...
			iLen += format(sBuffer[iLen], 2047 - iLen, "%2d %-22.22s %10d %3d^n", iRank, szName, iKills, iHS)
			
			SQL_NextRow(Query);
		}
	}
	else if(Data[1] == CSW_HEGRENADE)
	{
		iLen = format(sBuffer, 2047, "<body bgcolor=#000000><font color=#FFB000><pre>")
		iLen += format(sBuffer[iLen], 2047 - iLen, "%2s %-22.22s %13s^n", "#", "Nick", lKills)
		...
		while(SQL_MoreResults(Query))
		{
			...
			iLen += format(sBuffer[iLen], 2047 - iLen, "%2d %-22.22s %13d^n", iRank, szName, iKills)
	...
	{
		iLen = format(sBuffer, 2047, "<body bgcolor=#000000><font color=#FFB000><pre>")
		iLen += format(sBuffer[iLen], 2047 - iLen, "%2s %-22.22s %6s %6s %6s %6s %4s %4s %4s^n", "#", "Nick", lKills, lDeaths, "HS", lHits, lShots, lEff, lAcc)
		...
		while(SQL_MoreResults(Query))
		{
			...
			iRank = SQL_ReadResult(Query, 6);
			
			iLen += format(sBuffer[iLen], 2047 - iLen, "%2d %-22.22s %6d %6d %6d %6d %4d %3.0f%% %3.0f%%^n", iRank, szName, iKills, iDeaths, iHS, iHits, iShots, accuracy(iShots, iHits), effec(iKills, iDeaths))
			
			SQL_NextRow(Query);
		}
	}

	new topname[32], weaponname[32];
	get_weaponname(Data[1], weaponname, charsmax(weaponname));
	replace_all(weaponname, charsmax(weaponname), "weapon_", "");
	ucfirst(weaponname);
	formatex(topname, charsmax(topname), "%s Top15", weaponname)
	
	show_motd(id, sBuffer, topname);
	
	return PLUGIN_HANDLED;
}

Float:accuracy(iShots, iHits)
{
	if (!iShots)
		return (0.0)
