            ...
            switch(g_snd_next[playerid]){
                ...
                case EIM_SND_START:{
                    if(g_p_target[playerid] != NO_TARGET){
						...
						log_amx("medshot: %i %s", playerid, is_user_alive(id) ? "zywy" : "niezywy");
                ...
                case EIM_SND_HEALING:{
                    if(g_p_target[playerid] != NO_TARGET){
                        if(g_snd_current[playerid] != EIM_SND_HEALING){
							...
							log_amx("medcharge4: %i %s", playerid, is_user_alive(id) ? "zywy" : "niezywy");
                ...
                case EIM_SND_UNABLE:{
                    ...
                    if(g_snd_current[playerid] == EIM_SND_UNABLE){
						...
						log_amx("medshotno1: %i %s", playerid, is_user_alive(id) ? "zywy" : "niezywy");
...
public reset_all(){
    for(new i=1; i<=g_max_players; i++){
		...
		g_p_dmg[i] = NO_DAMAGE
		emit_sound(i, CHAN_ITEM, "items/medcharge4.wav", EIM_SND_VOL, ATTN_NORM, SND_STOP, PITCH_NORM)
		emit_sound(i, CHAN_ITEM, "items/medshot4.wav", EIM_SND_VOL, ATTN_NORM, SND_STOP, PITCH_NORM)
    }
...
public player_killed(id){
	_UnSet(_alive, id)
	emit_sound(id, CHAN_ITEM, "items/medcharge4.wav", EIM_SND_VOL, ATTN_NORM, SND_STOP, PITCH_NORM)
	emit_sound(id, CHAN_ITEM, "items/medshot4.wav", EIM_SND_VOL, ATTN_NORM, SND_STOP, PITCH_NORM)
	stop_healing_process(id)
}
