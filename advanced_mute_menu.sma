...
public plugin_init()
{
	...
	register_clcmd("say /mute", "menu_show");
	register_clcmd("say_team /mute", "menu_show");
...
public unmute_menu(id)
{
	...
	for (new i = 1; i <= MAX_PLAYERS; i++) {
		...
		formatex(itemData, charsmax(itemData), "%i#%i", playerName[i], type);

		menu_additem(menu, playerName[i], itemData);
