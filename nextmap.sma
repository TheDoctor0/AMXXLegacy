...
public sayNextMap()
{
	...
	client_print_color(0, 0, "%L^x04 %s^x01.", LANG_PLAYER, "NEXT_MAP", name)
}

public sayCurrentMap()
{
	client_print_color(0, 0, "%L:^x04 %s^x01.", LANG_PLAYER, "PLAYED_MAP", g_currentMap)
}

public sayFFStatus()
{
	client_print_color(0, 0, "%L:^x04  %L^x01.", LANG_PLAYER, "FRIEND_FIRE", LANG_PLAYER, get_pcvar_num(g_mp_friendlyfire) ? "ON" : "OFF")
}
