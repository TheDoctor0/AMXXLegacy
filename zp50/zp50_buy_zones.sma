...
new cvar_buy_ammo_human, cvar_buy_ammo_cost_ammopacks
...
public message_status_icon(msg_id, msg_dest, msg_entity)
{
	...
	set_msg_arg_int(3, get_msg_argtype(3), 200)
...
public clcmd_buyammo(id)
{
	...
	if (LibraryExists(LIBRARY_AMMOPACKS, LibType_Library))
	{
		...
		{
			zp_colored_print(id, "Nie masz wystarczajacej ilosci^x04 AP^x01, aby kupic amunicje (wymagane^x04 %i AP^x01).", get_pcvar_num(cvar_buy_ammo_cost_ammopacks))
			return;
		}
	}
	...
	zp_colored_print(id, "Pomyslnie zakupiles amunicje do swojej broni.")
}
