//=========================================================================
#define CSW_WEAPON		CSW_SG552
new const g_weapon_entity[]=	"weapon_sg552"
new const g_weaponbox_model[]=	"models/w_sg552.mdl"

#define WEAPON_NAME		"M1 Garand"
...
#define ANIM_IDLE		0
#define ANIM_IDLE_EX		2
#define ANIM_SHOOT_1		11
#define ANIM_SHOOT_EX		13
#define ANIM_RELOAD		10
#define ANIM_DRAW		4

#define ANIM_CHANGE		6
#define ANIM_CHANGE_EX		8

#define BODY_NUMBER		0
new const MODELS[][]={
				"models/csobc/v_m1garand.mdl",
				"models/csobc/p_m1garand.mdl",
				"models/csobc/w_m1garand.mdl"
}
new const SOUNDS[][]= {
				"weapons/m1garand-1.wav",
				"weapons/m1garand-2.wav",
				"weapons/m1garand_shoota_empty.wav",
				"weapons/m1garand_clipin1.wav",
				"weapons/m1garand_clipin2.wav"
}
#define WEAPONLIST		"csobc_m1garand"
new const SPRITES[][]=	{
				"sprites/csobc/640hud7.spr",
				"sprites/csobc/640hud112.spr",
				"sprites/csobc/640hud113.spr"
}
//=========================================================================
#include <amxmodx>
#include <engine>
...
public ham_weapon_secondaryattack(weapon_entity) {
	...
	if(get_pdata_int(weapon_entity, 74, 4)&M1_MODE){
		...
		play_weapon_animation(id, get_pdata_int(weapon_entity,51,4)?ANIM_CHANGE_EX:ANIM_CHANGE_EX+1)
		set_pdata_int(id,363, 90, 5)
	}else{
		set_pdata_int(weapon_entity, 74, get_pdata_int(weapon_entity, 74, 4)|M1_MODE, 4)
		play_weapon_animation(id, get_pdata_int(weapon_entity,51,4)?ANIM_CHANGE:ANIM_CHANGE+1)
...
public ham_weapon_primaryattack(weapon_entity) {
	...
	if(!clip){
		set_pdata_float(weapon_entity,46, 0.2, 4)
		emit_sound(id, CHAN_WEAPON, SOUNDS[2], 0.8, ATTN_NORM, 0, PITCH_NORM)
		return HAM_SUPERCEDE
	}
	ExecuteHam(Ham_Weapon_PrimaryAttack, weapon_entity)
	if(clip<=get_pdata_int(weapon_entity,51,4))return HAM_IGNORED
	emit_sound(id, CHAN_WEAPON, get_pdata_int(weapon_entity,51,4)?SOUNDS[0]:SOUNDS[1], VOL_NORM, ATTN_NORM, 0, PITCH_NORM)
	if(get_pdata_int(weapon_entity, 74, 4)&M1_MODE)
		play_weapon_animation(id, get_pdata_int(weapon_entity,51,4)?ANIM_SHOOT_EX:ANIM_SHOOT_EX+1)
	else    play_weapon_animation(id, get_pdata_int(weapon_entity,51,4)?ANIM_SHOOT_1:ANIM_SHOOT_1+1)
...
public ham_weapon_idle(ent) {
	...
	if(get_pdata_int(ent, 74, 4)&M1_MODE)
		play_weapon_animation(id, get_pdata_int(ent,51,4)?ANIM_IDLE_EX:ANIM_IDLE_EX+1)
	else    play_weapon_animation(id, get_pdata_int(ent,51,4)?ANIM_IDLE:ANIM_IDLE+1)
...
public ham_traceattack_post(pEntity,attacker,Float:flDamage,Float:direction[3],ptr,damage_type) {
	...
	if(get_pdata_int(weapon_entity, 74, 4)&M1_MODE){
		fm_get_aim_origin(attacker, vecEnd)
		set_tr2(ptr, TR_vecEndPos, vecEnd)
	}
...
public plugin_natives()
	register_native("zp_give_item_m1garand", "zp_give_item_m1garand", 1);
public zp_give_item_m1garand(id)
{
	new Ent=give_weapon(id)
	set_pdata_int(id,376+get_pdata_int(Ent,49,4),AMMO,5)
}
