...
#define ANIM_SHOOT_EX		2
#define ANIM_RELOAD		3
#define ANIM_DRAW		4

#define BODY_NUMBER		0
new const MODELS[][]={
				"models/csobc/v_balrog3.mdl",
				"models/csobc/p_balrog3.mdl",
				"models/csobc/w_balrog3.mdl"
}
new const SOUNDS[][]= {
...
#define WEAPONLIST		"csobc_balrog3"
new const SPRITES[][]=	{
				"sprites/csobc/640hud3.spr",
				"sprites/csobc/640hud86.spr"
}
//=========================================================================
#include <amxmodx>
#include <engine>
...
new g_msgWeaponList,g_wpn_variables[10],g_iszWeaponKey,g_index_smoke,g_index_shell,g_itemid, bool:g_charge[33], g_iExplode1, g_iExplode2
...
public plugin_precache() {
		...
		g_iExplode1=precache_model("sprites/csobc/balrog5stack.spr")
		g_iExplode2=precache_model("sprites/eexplo.spr")
...
public ham_traceattack_post(pEntity,attacker,Float:flDamage,Float:direction[3],ptr,damage_type) {
	...
	if(get_pdata_int(weapon_entity,51,4)<=15)
	{
		message_begin(MSG_BROADCAST,SVC_TEMPENTITY)
		write_byte(TE_EXPLOSION)
		engfunc(EngFunc_WriteCoord, vecEnd[0])
...
public ham_takedamage(victim,inflictor,attacker,Float:damage,damage_type){
	...
	if(get_pdata_int(weapon_ent,51,4)<=15) SetHamParamFloat(4,damage*DAMAGE+random_float(50.0,150.0))
	else SetHamParamFloat(4,damage*DAMAGE)
	}
}
...
public plugin_natives()
	register_native("zp_give_item_balrog3", "zp_give_item_balrog3", 1);
public zp_give_item_balrog3(id)
{
	new Ent=give_weapon(id)
	set_pdata_int(id,376+get_pdata_int(Ent,49,4),AMMO,5)
}
...
public damage_pre(attacker)
{
	static weapon_ent; weapon_ent=get_pdata_cbase(attacker,373,5)
	if(is_valid_weapon(weapon_ent)) {
		damage_set(attacker, (get_pdata_int(weapon_ent, 51, 4)<=15)?DAMAGE_EX:DAMAGE)
		return 1
	}
	return 0
}