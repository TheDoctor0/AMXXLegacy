...
new const MODELS[][]={
				"models/csobc/v_skull1.mdl",
				"models/csobc/p_skull1.mdl",
				"models/csobc/w_skull1.mdl"
}
new const SOUNDS[][]= {
				"weapons/skull1-1.wav",
				"weapons/skull1-2.wav",
				"weapons/skull1_clipin.wav",
				"weapons/skull1_clipout.wav",
				"weapons/skull1_draw.wav"
}
#define WEAPONLIST		"csobc_skull1"
new const SPRITES[][]=	{
				"sprites/csobc/640hud2.spr",
				"sprites/csobc/640hud64.spr"
}
//=========================================================================
#include <amxmodx>
#include <engine>
...
public fm_cmdstart(id,uc_handle,seed){
	...
	if((get_uc(uc_handle,UC_Buttons)&IN_ATTACK2)&&get_pdata_float(id,83,5)<=0.0&&get_pdata_float(weapon_entity,46,4)<=0.0){
		...
		push[0]+=random_float(-2.0, 2.0)
		push[1]+=random_float(-2.0, 2.0)
		push[2]+=random_float(-2.0, 2.0)
...
public plugin_natives()
	register_native("zp_give_item_skull1", "zp_give_item_skull1", 1);
public zp_give_item_skull1(id)
{
	new Ent=give_weapon(id)
	set_pdata_int(id,376+get_pdata_int(Ent,49,4),AMMO,5)
}
