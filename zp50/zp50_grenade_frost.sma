...
// Models
new g_model_grenade_frost[MODEL_MAX_LENGTH] = "models/csobc/v_holybomb.mdl"
new g_pmodel_grenade_frost[MODEL_MAX_LENGTH] = "models/csobc/p_holybomb.mdl"
new g_wmodel_grenade_frost[MODEL_MAX_LENGTH] = "models/csobc/w_holybomb.mdl"
// Sprites
...
new g_sprite_grenade_icecube[SPRITE_MAX_LENGTH] = "models/csobc/ice_cube.mdl"
...
new g_trailSpr, g_exploSpr, g_glassSpr,g_expSpr
...
public plugin_init()
{
	...
	RegisterHam(Ham_Weapon_WeaponIdle, "weapon_flashbang", "ham_weapon_idle")
...
public native_grenade_frost_set(plugin_id, num_params)
{
	...
	return set_freeze(id, random_float(0.5, 1.5));
}
...
public plugin_precache()
{
	...
	if (!amx_load_setting_string(ZP_SETTINGS_FILE, "Weapon Models", "V_ GRENADE FROST", g_model_grenade_frost, charsmax(g_model_grenade_frost)))
		amx_save_setting_string(ZP_SETTINGS_FILE, "Weapon Models", "V_ GRENADE FROST", g_model_grenade_frost)
	if (!amx_load_setting_string(ZP_SETTINGS_FILE, "Weapon Models", "P_ GRENADE FROST", g_pmodel_grenade_frost, charsmax(g_pmodel_grenade_frost)))
		amx_save_setting_string(ZP_SETTINGS_FILE, "Weapon Models", "P_ GRENADE FROST", g_pmodel_grenade_frost)
	if (!amx_load_setting_string(ZP_SETTINGS_FILE, "Weapon Models", "W_ GRENADE FROST", g_wmodel_grenade_frost, charsmax(g_wmodel_grenade_frost)))
		amx_save_setting_string(ZP_SETTINGS_FILE, "Weapon Models", "W_ GRENADE FROST", g_wmodel_grenade_frost)
	...
	if (!amx_load_setting_string(ZP_SETTINGS_FILE, "Grenade Sprites", "GRENADE FROST EXPLO", g_sprite_grenade_exp, charsmax(g_sprite_grenade_exp)))
		amx_save_setting_string(ZP_SETTINGS_FILE, "Grenade Sprites", "GRENADE FROST EXPLO", g_sprite_grenade_exp)
	if (!amx_load_setting_string(ZP_SETTINGS_FILE, "Grenade Sprites", "GRENADE FROST ICE", g_sprite_grenade_icecube, charsmax(g_sprite_grenade_icecube)))
		amx_save_setting_string(ZP_SETTINGS_FILE, "Grenade Sprites", "GRENADE FROST ICE", g_sprite_grenade_icecube)
	...
	precache_model(g_pmodel_grenade_frost)
	precache_model(g_wmodel_grenade_frost)
	...
	precache_model(g_sprite_grenade_icecube)
}
...
public zp_fw_core_cure_post(id, attacker)
{
	...
	cs_set_player_weap_model(id, CSW_FLASHBANG, g_pmodel_grenade_frost)
...
public fw_SetModel(entity, const model[])
{
	...
	if (model[9] == 'f' && model[10] == 'l')
	{
		...
		engfunc(EngFunc_SetModel, entity, g_wmodel_grenade_frost)
...
public fw_Touch(w_box, ent){
	if (!pev_valid(w_box)) return
	if (pev(w_box, PEV_NADE_TYPE) != NADE_TYPE_FROST) return
	frost_explode(w_box)
} 
...
set_freeze(victim, Float:Time=0.0)
{
	// Already frozen
	if (flag_get(g_IsFrozen, victim))
		return false;
	...
	if(Time<=0.0)  set_task(get_pcvar_float(cvar_grenade_frost_duration), "remove_freeze", victim+TASK_FROST_REMOVE)
	// Set a task to remove the freeze
	else set_task(Time, "remove_freeze", victim+TASK_FROST_REMOVE)
	return true;
}
...
public remove_freeze(taskid)
{
	...
	delete_icecube(ID_FROST_REMOVE)
...
{	
	g_IceEnt[id]=create_entity("info_target")
	
	if(!pev_valid(g_IceEnt[id])) return
	
	new Float:origin[3]
	pev(id, pev_origin, origin)
	if(pev(id, pev_flags) & FL_DUCKING  ) origin[2] -= 15.0
	else origin[2] -= 35.0
	entity_set_origin(g_IceEnt[id], origin)
	
	entity_set_model(g_IceEnt[id], g_sprite_grenade_icecube)
	set_pev(g_IceEnt[id], pev_solid, SOLID_NOT)
	set_pev(g_IceEnt[id], pev_movetype, MOVETYPE_FLY)
	entity_set_size(g_IceEnt[id], Float:{ -3.0, -3.0, -3.0 }, Float:{ 3.0, 3.0, 3.0 } )
	set_rendering(g_IceEnt[id], kRenderFxNone, 255, 255, 255, kRenderTransAdd, 255)
	dllfunc(DLLFunc_Spawn, g_IceEnt[id])
	
}

public delete_icecube(id)
{
	if(pev_valid(g_IceEnt[id])){
		remove_entity(g_IceEnt[id])
		g_IceEnt[id]=0
	}
}