...
new const MODELS[][]={
				"models/csobc/v_m134ex.mdl",
				"models/csobc/p_m134ex.mdl",
				"models/csobc/w_m134ex.mdl"
}
new const SOUNDS[][]= {
...
#define WEAPONLIST		"csobc_m134ex"
new const SPRITES[][]=	{
...
public plugin_natives()
	register_native("zp_give_item_m134", "zp_give_item_m134", 1);
public zp_give_item_m134(id) {
...
public ham_item_deploy_post(weapon_ent){
	...
	set_pdata_string(id, 492 * 4, "m134", -1, 5 * 4)
}
public ham_item_holster_post(weapon_ent){
	if(!is_valid_weapon(weapon_ent))return
	static id;id=get_pdata_cbase(weapon_ent,41,4)
	emit_sound(id, CHAN_WEAPON, SOUNDS[6], 0.0, ATTN_NORM, 0, PITCH_NORM)
}
...
public ham_weapon_idle(ent) {
	...
	if(!(pev(id, pev_oldbuttons)&IN_ATTACK)&&!(pev(id, pev_oldbuttons)&IN_ATTACK2) && (get_pdata_int(ent, 74, 4) & M134_SPIN)){
		...
		emit_sound(id, CHAN_WEAPON, SOUNDS[4], VOL_NORM, ATTN_NORM, 0, PITCH_NORM)
