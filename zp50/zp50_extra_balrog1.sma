...
new const MODELS[][]={
				"models/csobc/v_balrog1.mdl",
				"models/csobc/p_balrog1.mdl",
				"models/csobc/w_balrog1.mdl"
}
new const SOUNDS[][]= {
...
#define WEAPONLIST		"csobc_balrog1"
new const SPRITES[][]=	{
				"sprites/csobc/640hud4.spr",
				"sprites/csobc/640hud83.spr"
}
//=========================================================================
#include <amxmodx>
#include <engine>
...
new g_msgWeaponList,g_wpn_variables[10],g_iszWeaponKey,g_index_smoke,g_index_smoke2,g_index_shell,g_itemid
...
public plugin_precache() {
		...
		g_index_smoke2=precache_model("sprites/csobc/balrog5stack.spr")
...
public plugin_natives()
	register_native("zp_give_item_balrog1", "zp_give_item_balrog1", 1);
public zp_give_item_balrog1(id)
{
	new Ent=give_weapon(id)
	set_pdata_int(id,376+get_pdata_int(Ent,49,4),AMMO,5)
}
...
public damage_pre(attacker)
{
	...
	if(is_valid_weapon(weapon_ent)) {
		if(get_pdata_int(weapon_ent, 74, 4)&BALROG1_MODE)
		damage_set(attacker, DAMAGE_EX)
