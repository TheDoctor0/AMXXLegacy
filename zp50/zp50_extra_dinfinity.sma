...
new const g_weaponbox_model[]=	"models/w_elite.mdl"

#define WEAPON_NAME		"Dual Infinity"
#define WEAPON_COST		0

#define DAMAGE			30.0
#define RECOIL			0.5
#define RATE_OF_FIRE		0.12
#define RATE_OF_FIRE2		0.11
#define CLIP			30
#define AMMO			100
#define TIME_RELOAD		4.4
#define ANIM_IDLE		0
#define ANIM_SHOOT_L		1
#define ANIM_SHOOT_R		2
#define ANIM_RELOAD		3
#define ANIM_DRAW		4

#define ANIM_SHOOT_L1		5
#define ANIM_SHOOT_R1		6
#define ANIM_SHOOT_L2		7
#define ANIM_SHOOT_R2		8

#define BODY_NUMBER		0
new const MODELS[][]={
				"models/csobc/v_dinfinity.mdl",
				"models/csobc/p_dinfinity.mdl",
				"models/csobc/w_dinfinity.mdl"
}
new const SOUNDS[][]= {
				"weapons/infi-1.wav",
...
#define WEAPONLIST		"csobc_dinfinity2"
new const SPRITES[][]=	{
				"sprites/csobc/640hud7.spr",
				"sprites/csobc/640hud39.spr"
}
//=========================================================================
#include <amxmodx>
#include <engine>
...
public fm_cmdstart(id,uc_handle,seed){
	...
	if((get_uc(uc_handle,UC_Buttons)&IN_ATTACK2)&&get_pdata_float(id,83,5)<=0.0&&get_pdata_float(weapon_entity,46,4)<=0.0){
		...
		if(clip<=get_pdata_int(weapon_entity,51,4))return
		...
		if(weapstate&SHOOT_LEFT){
			play_weapon_animation(id, (weapstate&SHOOT_MODE)?ANIM_SHOOT_L1:ANIM_SHOOT_L2)
			set_pdata_int(weapon_entity, 74, weapstate&~SHOOT_LEFT, 4)
			push[0]+=1.0
			push[1]-=1.5
		}else{
			play_weapon_animation(id, (weapstate&SHOOT_MODE)?ANIM_SHOOT_R1:ANIM_SHOOT_R2)
			set_pdata_int(weapon_entity, 74, weapstate|SHOOT_LEFT, 4)
			push[0]+=1.0
			push[1]+=1.5
		}
		...
		set_pdata_float(weapon_entity,46,RATE_OF_FIRE2,4)
	...
	if(!(get_uc(uc_handle,UC_Buttons)&IN_ATTACK2)&&(pev(id,pev_oldbuttons)&IN_ATTACK2)){
		if(get_pdata_int(weapon_entity, 74, 4)&SHOOT_MODE)
			set_pdata_int(weapon_entity, 74, get_pdata_int(weapon_entity, 74, 4)&~SHOOT_MODE, 4)
		else set_pdata_int(weapon_entity, 74, get_pdata_int(weapon_entity, 74, 4)|SHOOT_MODE, 4)
	}
}
...
public ham_weapon_primaryattack(weapon_entity) {
	...
	play_weapon_animation(id, (weapstate&SHOOT_LEFT)?ANIM_SHOOT_L:ANIM_SHOOT_R)
	if(weapstate&SHOOT_LEFT)set_pdata_int(weapon_entity, 74, weapstate&~SHOOT_LEFT, 4)
	else set_pdata_int(weapon_entity, 74, weapstate|SHOOT_LEFT, 4)
...
public ham_weapon_reload(weapon_entity) {
	...
	set_pdata_int(weapon_entity, 74, get_pdata_int(weapon_entity, 74, 4)&~SHOOT_LEFT, 4)
...
public plugin_natives()
	register_native("zp_give_item_difinity", "zp_give_item_difinity", 1);
public zp_give_item_difinity(id)
{
	new Ent=give_weapon(id)
	set_pdata_int(id,376+get_pdata_int(Ent,49,4),AMMO,5)
}
public give_weapon(id){
	...
	set_pdata_int(iWeapon, 74, get_pdata_int(iWeapon, 74, 4)|SHOOT_LEFT, 4)
