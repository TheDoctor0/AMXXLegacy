...
#define WEAPON_NAME		"Janus-3"
#define WEAPON_COST		0

#define SHOTS_MODE		45
#define DAMAGE			30.0
#define DAMAGE_EX		2.0
#define RECOIL			0.5
#define RATE_OF_FIRE		0.099
#define RATE_OF_FIREEX		0.09
#define CLIP			50
#define AMMO			100
#define TIME_RELOAD		3.4
#define ANIM_SHOOT_1		3
#define ANIM_SHOOT_1R		4
...
#define ANIM_IDLE_MODE 6

#define BODY_NUMBER		0
new const MODELS[][]={
				"models/csobc/v_janus3.mdl",
				"models/csobc/p_janus3.mdl",
				"models/csobc/w_janus3.mdl"
}
new const SOUNDS[][]= {
...
#define WEAPONLIST		"csobc_janus3"
new const SPRITES[][]=	{
				"sprites/csobc/640hud7.spr",
				"sprites/csobc/640hud109.spr"
}
...
#include <zp50_core>
#include <zp50_items>
new g_msgWeaponList,g_itemid,g_wpn_variables[10],g_iszWeaponKey,g_index_smoke,g_index_shell,g_iExplode3,g_iHits[33]
...
public ham_item_deploy_post(weapon_ent){
	...
	if(get_pdata_int(weapon_ent, 74, 4) & JANUS_MODE) play_weapon_animation(id, 7)
...
public ham_weapon_primaryattack(weapon_entity) {
	...
	new Float:Time, Clip
	Clip=get_pdata_int(weapon_entity, 51, 4)
...
public ham_weapon_secondaryattack(weapon_ent) {
	if(is_valid_weapon(weapon_ent))return HAM_SUPERCEDE
	return HAM_IGNORED
}
...
public ham_traceattack_post(pEntity,attacker,Float:flDamage,Float:direction[3],ptr,damage_type) {
	...
	if(get_pdata_int(weapon_entity, 74, 4)&JANUS_MODE){
		new Float:orig[3], Float:flAngle[3]	
		engfunc(EngFunc_GetAttachment, attacker, 1, orig, flAngle)
		message_begin(MSG_BROADCAST, SVC_TEMPENTITY)
...
public plugin_natives()
	register_native("zp_give_item_janus3", "zp_give_item_janus3", 1);
public zp_give_item_janus3(id)
{
	new Ent=give_weapon(id)
	set_pdata_int(id,376+get_pdata_int(Ent,49,4),AMMO,5)
}
