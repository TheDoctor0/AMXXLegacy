...
#define WEAPON_NAME		"Sha Wujing Dual Handgun"
...
#define ANIM_SHOOT_L1		3
#define ANIM_SHOOT_R1		4
#define ANIM_RELOAD		5
#define ANIM_DRAW		6

#define BODY_NUMBER		0
new const MODELS[][]={
				"models/csobc/v_monkeywpnset2.mdl",
				"models/csobc/p_monkeywpnset2.mdl",
				"models/csobc/w_monkeywpnset2.mdl"
}
new const SOUNDS[][]= {
...
#define WEAPONLIST		"csobc_monkeywpnset2"
new const SPRITES[][]=	{
				"sprites/csobc/640hud7.spr",
				"sprites/csobc/640hud106.spr"
}
//=========================================================================
#include <amxmodx>
#include <engine>
...
public fm_cmdstart(id,uc_handle,seed){
	...
	if((get_uc(uc_handle,UC_Buttons)&IN_ATTACK2)&&get_pdata_float(id,83,5)<=0.0&&get_pdata_float(weapon_entity,46,4)<=0.0){
		...
		if(weapstate&SHOOT_LEFT){
			play_weapon_animation(id, ANIM_SHOOT_L1)
			set_pdata_int(weapon_entity, 74, weapstate&~SHOOT_LEFT, 4)
			push[0]+=1.0
			push[1]-=1.5
		}else{
			play_weapon_animation(id, ANIM_SHOOT_R1)
			set_pdata_int(weapon_entity, 74, weapstate|SHOOT_LEFT, 4)
			push[0]+=1.0
			push[1]+=1.5
		}
...
public plugin_natives()
	register_native("zp_give_item_monkeyswpnset2", "zp_give_item_monkeyswpnset2", 1);
public zp_give_item_monkeyswpnset2(id)
{
	new Ent=give_weapon(id)
	set_pdata_int(id,376+get_pdata_int(Ent,49,4),AMMO,5)
}
