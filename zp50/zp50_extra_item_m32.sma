...
new const MODELS[][]={
				"models/csobc/v_m32.mdl",
				"models/csobc/p_m32.mdl",
				"models/csobc/w_m32.mdl"
}
new const SOUNDS[][]= {
				"weapons/m32-1.wav",
				"weapons/m32_after_reload.wav",
				"weapons/m32_explosion.wav",
				"weapons/m32_insert.wav",
				"weapons/m32_start_reload.wav"
}
#define WEAPONLIST		"csobc_m32"
new const SPRITES[][]=	{
				"sprites/csobc/640hud7.spr",
				"sprites/csobc/640hud75.spr",
				"sprites/csobc/scope_grenade.spr"
}
...
new g_msgWeaponList,g_itemid,g_wpn_variables[10],g_iszWeaponKey,decal_index, Float:g_fTimeZoom[33]
new trail_spr, explode_spr, smoke_spr
new Blood[2]

new gMsg_SetFOV, gMsg_CurWeapon, gMsg_HideWeapon, gMsg_Crosshair
...
public plugin_init() {
		...
		register_touch("m32_grenade", "*", "fwTouch")
...
public fm_cmdstart(id,uc_handle,seed){
	...
	if((get_uc(uc_handle,UC_Buttons)&IN_ATTACK2)&&get_gametime()>=g_fTimeZoom[id]){
		g_fTimeZoom[id]=get_gametime()+0.2
		if(get_pdata_int(id, 363, 5) == 90) set_pdata_int(id, 363, 55, 5)
		else {
			new Clip=get_pdata_int(weapon_ent, 51, 4)
			Msg_SetFOV(id, 89)
			Msg_CurWeapon(id, 1, CSW_BALROG7, Clip)
			Msg_SetFOV(id, 90)
...
public plugin_natives()
	register_native("zp_give_item_m32", "zp_give_item_m32", 1);
public zp_give_item_m32(id)
{
	new Ent=give_weapon(id)
	set_pdata_int(id,376+get_pdata_int(Ent,49,4),AMMO,5)
}
