...
#define WEAPON_NAME		"Balrog-7"
#define WEAPON_COST		0

#define DAMAGE			35.0
#define RECOIL			0.5
#define RATE_OF_FIRE		0.11
#define CLIP			120
#define AMMO			200
...
new const MODELS[][]={
				"models/csobc/v_balrog7.mdl",
				"models/csobc/p_balrog7.mdl",
				"models/csobc/w_balrog7.mdl"
}
new const SOUNDS[][]= {
...
#define WEAPONLIST		"csobc_balrog7"
new const SPRITES[][]=	{
				"sprites/csobc/640hud2.spr",
				"sprites/csobc/640hud76.spr"
}
//=========================================================================
#include <amxmodx>
#include <engine>
...
public ham_weapon_primaryattack(weapon_entity) {
	...
	if(get_pdata_int(weapon_entity, 64, 4)==10)
	{
		g_charge[id]=true
...
public ham_traceattack_post(pEntity,attacker,Float:flDamage,Float:direction[3],ptr,damage_type) {
	...
	{
		g_charge[attacker]=false
		
		message_begin(MSG_BROADCAST, SVC_TEMPENTITY)
		write_byte(TE_EXPLOSION)
		engfunc(EngFunc_WriteCoord, vecEnd[0])
		engfunc(EngFunc_WriteCoord, vecEnd[1])
		engfunc(EngFunc_WriteCoord, vecEnd[2]+25)
		...
		engfunc(EngFunc_WriteCoord, vecEnd[1])
		engfunc(EngFunc_WriteCoord, vecEnd[2]+15)
		write_short(g_iExplode2) // Sprite index
		write_byte(20) // Scale
		...
		{	
			...
			{
				damage=random_float(250.0, 350.0)
...
public plugin_natives()
	register_native("zp_give_item_balrog7", "zp_give_item_balrog7", 1);
public zp_give_item_balrog7(id)
{
	new Ent=give_weapon(id)
	set_pdata_int(id,376+get_pdata_int(Ent,49,4),AMMO,5)
}
