...
new const g_weapon_entity[]=	"weapon_awp"
new const g_weaponbox_model[]=	"models/w_awp.mdl"

#define WEAPON_NAME		"Mosin Nagant"
#define WEAPON_COST		0

#define DAMAGE			250.0
#define RECOIL			0.5
#define RATE_OF_FIRE		1.3
#define CLIP			5
#define AMMO			90
...
new const MODELS[][]={
				"models/csobc/v_mosin.mdl",
				"models/csobc/p_mosin.mdl",
				"models/csobc/w_mosin.mdl"
}
new const SOUNDS[][]= {
				"weapons/mosin-1.wav",
...
#define WEAPONLIST		"csobc_mosin"
new const SPRITES[][]=	{
				"sprites/csobc/640hud7.spr",
				"sprites/csobc/640hud110.spr"
}
//=========================================================================
#include <amxmodx>
#include <engine>
...
public ham_weapon_primaryattack(weapon_entity) {
	...
	set_pdata_float(weapon_entity, 48, RATE_OF_FIRE+0.7, 4)
...
public ham_item_postframe(weapon_entity)  {
	...
	static id; id = get_pdata_cbase(weapon_entity,41,4)
	if(get_pdata_float(weapon_entity, 48, 4)<=0.0&&get_pdata_int(weapon_entity, 55, 4)) Reload(id, weapon_entity)
}
...
public plugin_natives()
	register_native("zp_give_item_mosin", "zp_give_item_mosin", 1);
public zp_give_item_mosin(id)
{
	new Ent=give_weapon(id)
	set_pdata_int(id,376+get_pdata_int(Ent,49,4),AMMO,5)
}
