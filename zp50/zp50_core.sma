...
public client_putinserver(id) flag_unset(g_RespawnAsZombie, id)
...
CheckLastZombieHuman()
{
	...
	if (zombie_count == 1)
	{
		...
		{
			// Last zombie
			if (is_user_alive(id) && flag_get(g_IsZombie, id) && !flag_get(g_IsFirstZombie, id))
			{
	...
	if (human_count == 1 && GetAliveCount()>1)
	{
		for (id = 1; id <= g_MaxPlayers; id++)
		{
			// Last human
