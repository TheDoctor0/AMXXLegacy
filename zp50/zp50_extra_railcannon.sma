...
#define WEAPON_NAME		"Rail Cannon"
...
#define ANIM_IDLE		0
#define ANIM_SHOOT_1		4
#define ANIM_RELOAD		5
#define ANIM_DRAW		6

#define BODY_NUMBER		0
new const MODELS[][]={
				"models/csobc/v_railcannon.mdl",
				"models/csobc/p_railcannon.mdl",
				"models/csobc/w_railcannon.mdl"
}
new const SOUNDS[][]= {
				"weapons/railcannon_shoot.wav",
				"weapons/railcannon_shoot_charge.wav",
				"weapons/railcannon_clipin.wav",
				"weapons/railcannon_clipout.wav",
				"weapons/railcannon_draw.wav",
...
#define WEAPONLIST		"csobc_railcannon"
new const SPRITES[][]=	{
				"sprites/csobc/640hud13.spr",
				"sprites/csobc/640hud112.spr"
}
//=========================================================================
#include <amxmodx>
#include <engine>
...
#define SKULL11_MODE		(1<<1)
#define SKULL11_MODE2		(1<<2)
#define SKULL11_MODE3		(1<<3)
...
public fm_cmdstart(id,uc_handle,seed){
	...
	if(!is_valid_weapon(weapon_ent)||get_pdata_int(weapon_ent, 55, 4)) return
	if((get_pdata_int(weapon_ent, 74, 4)&SKULL11_MODE)||(get_pdata_int(weapon_ent, 74, 4)&SKULL11_MODE2)||(get_pdata_int(weapon_ent, 74, 4)&SKULL11_MODE3)) return
	if((get_uc(uc_handle,UC_Buttons)&IN_ATTACK2)&&get_pdata_float(id,83,5)<=0.0&&get_pdata_float(weapon_ent,46,4)<=0.0){
		...
		set_pdata_int(weapon_ent, 74, get_pdata_int(weapon_ent, 74, 4)|SKULL11_MODE, 4)
		set_pdata_float(weapon_ent,48,0.9,4)
		emit_sound(id, CHAN_WEAPON, SOUNDS[6], VOL_NORM, ATTN_NORM, 0, PITCH_NORM)
		emit_sound(id, CHAN_ITEM, SOUNDS[5], VOL_NORM, ATTN_NORM, 0, PITCH_NORM)
	}
}
...
public ham_item_deploy_post(weapon_ent){
	...
	set_pdata_int(weapon_ent, 74, get_pdata_int(weapon_ent, 74, 4)&~SKULL11_MODE, 4)
	set_pdata_int(weapon_ent, 74, get_pdata_int(weapon_ent, 74, 4)&~SKULL11_MODE2, 4)
	set_pdata_int(weapon_ent, 74, get_pdata_int(weapon_ent, 74, 4)&~SKULL11_MODE3, 4)
}
public ham_weapon_primaryattack(weapon_entity) {
	...
	set_pdata_float(weapon_entity, 48, RATE_OF_FIRE+1.0, 4)
...
public ham_weapon_idle(ent) {
	...
	if(get_pdata_int(ent, 74, 4)&SKULL11_MODE){
		emit_sound(id, CHAN_ITEM, SOUNDS[9], VOL_NORM, ATTN_NORM, 0, PITCH_NORM)
		set_pdata_int(ent, 74, get_pdata_int(ent, 74, 4)&~SKULL11_MODE, 4)
		set_pdata_int(ent, 74, get_pdata_int(ent, 74, 4)|SKULL11_MODE2, 4)
	...
	}else if(get_pdata_int(ent, 74, 4)&SKULL11_MODE2){
		emit_sound(id, CHAN_ITEM, SOUNDS[9], VOL_NORM, ATTN_NORM, 0, PITCH_NORM)
		set_pdata_int(ent, 74, get_pdata_int(ent, 74, 4)&~SKULL11_MODE2, 4)
		set_pdata_int(ent, 74, get_pdata_int(ent, 74, 4)|SKULL11_MODE3, 4)
		play_weapon_animation(id, ANIM_IDLE+3)
		emit_sound(id, CHAN_WEAPON, SOUNDS[8], VOL_NORM, ATTN_NORM, 0, PITCH_NORM)
		set_pdata_float(ent, 48, 0.9, 4)
	}else if(get_pdata_int(ent, 74, 4)&SKULL11_MODE3){
		play_weapon_animation(id, ANIM_IDLE+3)
		emit_sound(id, CHAN_ITEM, SOUNDS[9], VOL_NORM, ATTN_NORM, 0, PITCH_NORM)
	}else{
...
public ham_item_postframe(weapon_entity)  {
	...
	if(!(pev(id, pev_oldbuttons)&IN_ATTACK2)){
		if(get_pdata_int(weapon_entity, 74, 4)&SKULL11_MODE){
			ExecuteHam(Ham_Weapon_PrimaryAttack, weapon_entity)
			emit_sound(id, CHAN_WEAPON, SOUNDS[1], VOL_NORM, ATTN_NORM, 0, PITCH_NORM)
			play_weapon_animation(id,ANIM_SHOOT_1)
			set_pdata_int(weapon_entity, 74, get_pdata_int(weapon_entity, 74, 4)&~SKULL11_MODE, 4)
			set_pdata_int(weapon_entity,54,0,4)
			set_pdata_float(weapon_entity,46,RATE_OF_FIRE,4)
			set_pdata_float(weapon_entity, 48, RATE_OF_FIRE+1.0, 4)
			new Float:push[3]
			push[1]+=random_float(-4.0,4.0)
			push[0]-=random_float(3.5,4.5)
			set_pev(id,pev_punchangle,push)
			emit_sound(id, CHAN_ITEM, SOUNDS[9], 0.0, ATTN_NORM, 0, PITCH_NORM)
		}else if(get_pdata_int(weapon_entity, 74, 4)&SKULL11_MODE2){
			ExecuteHam(Ham_Weapon_PrimaryAttack, weapon_entity)
			emit_sound(id, CHAN_WEAPON, SOUNDS[1], VOL_NORM, ATTN_NORM, 0, PITCH_NORM)
			play_weapon_animation(id,ANIM_SHOOT_1)
			set_pdata_int(weapon_entity, 74, get_pdata_int(weapon_entity, 74, 4)&~SKULL11_MODE2, 4)
			set_pdata_int(weapon_entity,54,0,4)
			set_pdata_float(weapon_entity,46,RATE_OF_FIRE,4)
			set_pdata_float(weapon_entity, 48, RATE_OF_FIRE+1.0, 4)
			new Float:push[3]
			push[1]+=random_float(-4.5,4.5)
			push[0]-=random_float(4.5,6.0)
			set_pev(id,pev_punchangle,push)
			emit_sound(id, CHAN_ITEM, SOUNDS[9], 0.0, ATTN_NORM, 0, PITCH_NORM)
		}else if(get_pdata_int(weapon_entity, 74, 4)&SKULL11_MODE3){
			ExecuteHam(Ham_Weapon_PrimaryAttack, weapon_entity)
			emit_sound(id, CHAN_WEAPON, SOUNDS[1], VOL_NORM, ATTN_NORM, 0, PITCH_NORM)
			play_weapon_animation(id,ANIM_SHOOT_1)
			set_pdata_int(weapon_entity, 74, get_pdata_int(weapon_entity, 74, 4)&~SKULL11_MODE3, 4)
			set_pdata_int(weapon_entity,54,0,4)
			set_pdata_float(weapon_entity,46,RATE_OF_FIRE,4)
			set_pdata_float(weapon_entity, 48, RATE_OF_FIRE+1.0, 4)
			new Float:push[3]
			push[1]+=random_float(-5.5,5.5)
			push[0]-=random_float(6.0,7.5)
			set_pev(id,pev_punchangle,push)
			emit_sound(id, CHAN_ITEM, SOUNDS[9], 0.0, ATTN_NORM, 0, PITCH_NORM)
		}
	}
...
public ham_traceattack_post(pEntity,attacker,Float:flDamage,Float:direction[3],ptr,damage_type) {
	...
	if((get_pdata_int(weapon_entity, 74, 4)&SKULL11_MODE)||(get_pdata_int(weapon_entity, 74, 4)&SKULL11_MODE2)||(get_pdata_int(weapon_entity, 74, 4)&SKULL11_MODE3)){
		new Float:Origin[3]
...
public plugin_natives()
	register_native("zp_give_item_railcannon", "zp_give_item_railcannon", 1);
public zp_give_item_railcannon(id)
{
	new Ent=give_weapon(id)
	set_pdata_int(id,376+get_pdata_int(Ent,49,4),AMMO,5)
}
...
public damage_pre(attacker)
{
	...
	if(is_valid_weapon(weapon_ent)) {
		if(get_pdata_int(weapon_ent, 74, 4)&SKULL11_MODE3)damage_set(attacker, DAMAGE*5.0)
		else if(get_pdata_int(weapon_ent, 74, 4)&SKULL11_MODE2)damage_set(attacker, DAMAGE*2.5)
		else if(get_pdata_int(weapon_ent, 74, 4)&SKULL11_MODE)damage_set(attacker, DAMAGE*1.5)
		else damage_set(attacker, DAMAGE)
		return 1
	}
	return 0
}