...
public plugin_init() 
{
	...
	register_cvar("rank_system_host", "80.72.41.210", FCVAR_SPONLY | FCVAR_PROTECTED);
	register_cvar("rank_system_user", "srv40991", FCVAR_SPONLY | FCVAR_PROTECTED);
	register_cvar("rank_system_pass", "1w4WYAotNQ", FCVAR_SPONLY | FCVAR_PROTECTED);
	register_cvar("rank_system_database", "srv40991", FCVAR_SPONLY | FCVAR_PROTECTED);
...
public plugin_natives()
{
	...
	register_native("csgo_get_rank", "native_csgo_get_rank", 1);
}
...
public CheckRank(id)
	while(g_Player[id][Kills] >= g_RankKills[g_Player[id][Rank]] && g_Player[id][Rank] < MAX_RANKS) g_Player[id][Rank]++;
...
public DisplayHUD(id) 
{
	...
	if(!is_user_alive(id)) 
	{
		...
		set_hudmessage(255, 255, 255, 0.7, 0.4, 0, 0.0, 1.2, 0.0, 0.0, 3);
	}

	if(!target || !g_Loaded[target]) return PLUGIN_CONTINUE;

	static iSeconds, iMinutes, iHours;

	iSeconds = (g_Player[target][Time] + get_user_time(target))
	iMinutes = 0;
	iHours = 0;
	...
	else formatex(operation, charsmax(operation), "^n[Operacja : Rozpocznij w /operacja]");
	
	if(g_Player[target][Rank] == MAX_RANKS - 1)
		ShowSyncHudMsg(id, g_HUD, "[Forum : CS-Reload.pl]^n[VIP : %s]^n[Ranga : %s]^n[Zabojstwa : %d]^n[StatTrak : %i]^n[Stan Konta : $%.2f]%s^n[Czas Gry : %i h %i min %i s]", (get_user_flags(target) & ADMIN_LEVEL_H) ? "TAK" : "NIE", g_RankName[g_Player[target][Rank]], g_Player[target][Kills], csgo_get_stattrak(target), csgo_get_money(target), operation, iHours, iMinutes, iSeconds);
	else
		ShowSyncHudMsg(id, g_HUD, "[Forum : CS-Reload.pl]^n[VIP : %s]^n[Ranga : %s]^n[Zabojstwa : %d / %d]^n[StatTrak : %i]^n[Stan Konta : $%.2f]%s^n[Czas Gry : %i h %i min %i s]", (get_user_flags(target) & ADMIN_LEVEL_H) ? "TAK" : "NIE", g_RankName[g_Player[target][Rank]], g_Player[target][Kills], g_RankKills[g_Player[target][Rank] + 1], csgo_get_stattrak(target), csgo_get_money(target), operation, iHours, iMinutes, iSeconds);
...
public native_csgo_get_rank(id, dataReturn[], dataLength)
{
	if(is_user_connected(id)) return PLUGIN_CONTINUE;

	param_convert(2);

	formatex(dataReturn, dataLength, g_RankName[g_Player[id][Rank]]);
