...
new CheckTime, VoteTime, PauseTime, MPlayers, DPlayers, Maps, Type, MapChooser, LastMaps
new bool:SmallMap, bool:BigMap, bool:Voted, bool:Voting
...
public plugin_init() {
	...
	VoteTime = register_cvar("mdm_votetime", "170") // czas w sekundach do konca mapy, w ktorym jesli graczy jest graczy jest zbyt malo graczy zostanie wywolane vote jedynie z malymi mapami
	PauseTime = register_cvar("mdm_pausetime", "140") // czas w sekundach do konca mapy, w ktorym plugin zostanie dezaktywowany, by nie wchodzic w konflikt z mapchooserem (nie radze zmieniac na mniej niz 130)
	...
	LastMaps = register_cvar("mdm_lastmaps", "0") // ilosc map granych ostatnio, ktore nie pojawia sie w vote (dziala tylko jesli na serwerze jest plugin DeagsMapManager i cvar mdm_mapchooser jest ustawiony na 1)
...
public CheckPlayers(){
	if(Voted || Voting || get_playersnum() < 2 || get_timeleft() <= get_pcvar_num(PauseTime))
		return PLUGIN_CONTINUE
	if((get_playersnum() >= get_pcvar_num(DPlayers) && SmallMap) || (get_playersnum() < get_pcvar_num(MPlayers) && BigMap)){
		if(get_timeleft() <= get_pcvar_num(VoteTime)){
			if(SmallMap){
				SmallMap = false
				BigMap = true
				Choice = 2
				...
				ColorChat(0, RED, "[MDM]^x01 Zbyt malo graczy, by zagrac na^x04 duzej^x01 mapie! Za^x04 10 sekund^x01 nastapi glosowanie.")
			}
			Voting = true
			return PLUGIN_CONTINUE
		}
