...
public plugin_precache()
	precache_generic("sound/admin/wiadomosc.wav")
...
public cmdSayAdmin(id)
{
	...
	if(get_user_flags(id) & ADMIN_BAN)
		format(message, 189, "^x04(ADMIN)^x03 %s^x04 :  %s", name, message[1])
	else if(get_user_flags(id) & ADMIN_LEVEL_H)
		format(message, 189, "^x04(VIP)^x03 %s^x04 :  %s", name, message[1])
	else
		format(message, 189, "^x04(GRACZ)^x03 %s^x04 :  %s", name, message[1])
	...
	for (new i = 0; i < inum; ++i)
	{
		...
		if (players[i] != id && get_user_flags(players[i]) & ADMIN_CHAT){
			client_print_color(players[i], id, message)
			client_cmd(players[i], "spk sound/admin/wiadomosc.wav")
		}
	}

	client_print_color(id, id, message)
	client_cmd(id, "spk sound/admin/wiadomosc.wav")
...
public cmdChat(id, level, cid)
{
	...
	new name[MAX_NAME_LENGTH], players[MAX_PLAYERS], inum, authid[32]
...
public cmdPsay(id, level, cid)
{
	...
	if (id && id != priv)
		client_print_color(id, id, "^x04(PRIV)^x03 %s do %s^x04 :  %s", name2, name, message[length])
	// client_print(id, print_chat, "(%s) %s :   %s", name, name2, message[length])
	
	client_print_color(priv, id, "^x04(PRIV)^x03 %s do %s^x04 :  %s", name2, name, message[length])
	client_cmd(priv, "spk sound/admin/wiadomosc.wav")
