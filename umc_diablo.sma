...
stock bool:isValidMap(szMap[], iNom = 0)
{
	...
	{
		//log_to_file("addons/amxmodx/logs/umc.log", "----- Checking if map is valid -----", LastMaps[MapNum], szMap);
		for(new MapNum; MapNum < get_pcvar_num(Cvar_LastMaps); MapNum++) 
		{
			if(equal(LastMaps[MapNum], szMap))
				return false;
			//log_to_file("addons/amxmodx/logs/umc.log", "Last Map: %s | Map: %s", LastMaps[MapNum], szMap);
		}
