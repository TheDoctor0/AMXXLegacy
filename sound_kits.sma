...
new const soundCmds[][] = { "say /kit", "say_team /kit", "say /soundkit", "say_team /soundkit", "say /soundkits", "say_team /soundkits", "say /soundkity", "say_team /soundkity" };

enum _:soundData { SOUND_PRICE, SOUND_TYPE, SOUND_NAME[64], SOUND_PATH[128] };
enum _:soundTypes { SOUND_NONE = -1, SOUND_MP3, SOUND_WAV };

new Array:soundKits, Array:playerSoundKits[MAX_PLAYERS + 1], playerActiveSoundKit[MAX_PLAYERS + 1], safePlayerName[MAX_PLAYERS + 1][32], soundKitsLoaded, Handle:sql;
...
{
	for (new i = 1; i <= MAX_PLAYERS; i++) playerSoundKits[i] = ArrayCreate();
...
public player_death()
{
	...
	if (playerActiveSoundKit[killer] > SOUND_NONE) {
		...
		client_cmd(victim, "%s %s", soundKit[SOUND_TYPE] == SOUND_MP3 ? "mp3 play" : "spk", soundKit[SOUND_PATH]);
	}
}
...
public sound_kits(id)
{
	if (!get_bit(id, soundKitsLoaded)) {
		client_print_color(id, id, "^x04[SOUNDKIT]^x01 Trwa ladowanie twoich danych.");
...
{
	if (!ArraySize(soundKits)) return PLUGIN_HANDLED;

	if (ArraySize(soundKits) == ArraySize(playerSoundKits[id])) {
		client_print_color(id, id, "^x04[SOUNDKIT]^x01 Posiadasz juz wszystkie dostepne^x03 sound kity^x01.");
...
public buy_sound_kit_confirm_handle(id, menu, item)
{
	...
	if (!item) {
		if (get_user_money(id) < soundKit[SOUND_PRICE]) {
			client_print_color(id, id, "^x04[SOUNDKIT]^x01 Nie posiadasz wystarczajacych srodkow, aby zakupic ten^x03 sound kit^x01.");
		...
		client_print_color(id, id, "^x04[SOUNDKIT]^x01 Kupiles sound kit:^x03 %s^x01.", soundKit[SOUND_NAME]);

		menu_destroy(menu);
	} else {
		client_cmd(id, "%s %s", soundKit[SOUND_TYPE] == SOUND_MP3 ? "mp3 play" : "spk", soundKit[SOUND_PATH]);

		client_print_color(id, id, "^x04[SOUNDKIT]^x01 Odtwarzam sound kit:^x03 %s^x01.", soundKit[SOUND_NAME]);
...
public set_sound_kit(id)
{
	if (!ArraySize(playerSoundKits[id])) {
		client_print_color(id, id, "^x04[SOUNDKIT]^x01 Nie posiadasz zadnych^x03 sound kitow^x01.");
...
public set_sound_kit_handle(id, menu, item)
{
	...
	if (soundKitId == SOUND_NONE) client_print_color(id, id, "^x04[SOUNDKIT]^x01 Sound kit zostal^x03 wylaczony^x01.");
	else {
		ArrayGetArray(soundKits, soundKitId, soundKit);

		client_print_color(id, id, "^x04[SOUNDKIT]^x01 Ustawiles sound kit:^x03 %s^x01.", soundKit[SOUND_NAME]);
	}
...
{
	new playerId[1], queryData[128], playerName[32];

	playerId[0] = id;

	get_user_name(id, playerName, charsmax(playerName));

	mysql_safe_string(playerName, safePlayerName[id], charsmax(safePlayerName[]));

	formatex(queryData, charsmax(queryData), "SELECT * FROM `sound_kits` WHERE name = ^"%s^";", safePlayerName[id]);
...
public add_player_sound_kit(id, soundKitId)
{
	...
	formatex(queryData, charsmax(queryData), "INSERT INTO `sound_kits` (`name`, `sound_kit`) VALUES (^"%s^", ^"%s^");", safePlayerName[id], safeSoundKitName);

	SQL_ThreadQuery(sql, "ignore_handle", queryData);

	if (playerActiveSoundKit[id] == SOUND_NONE) set_player_sound_kit(id, soundKitId);
}
...
public set_player_sound_kit(id, soundKitId)
{
	...
	if (soundKitId > SOUND_NONE) {
		new soundKit[soundData], safeSoundKitName[64];

		ArrayGetArray(soundKits, soundKitId, soundKit);

		mysql_safe_string(soundKit[SOUND_NAME], safeSoundKitName, charsmax(safeSoundKitName));

		formatex(queryData, charsmax(queryData), "UPDATE `sound_kits` SET `active` = '1' WHERE `name` = ^"%s^" AND `sound_kit` = ^"%s^";", safePlayerName[id], safeSoundKitName);
	} else formatex(queryData, charsmax(queryData), "UPDATE `sound_kits` SET `active` = '0' WHERE `name` = ^"%s^";", safePlayerName[id]);
