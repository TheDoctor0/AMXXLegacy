...
new sFlags[25], sName[33][64], bool:bAdmin[33], bool:bSQL, Handle:hSqlHook;
...
public sql_init()
{
	...
	get_cvar_string("sklepsms_host", sHost, charsmax(sHost));
	get_cvar_string("sklepsms_user", sUser, charsmax(sUser));
	get_cvar_string("sklepsms_pass", sPass, charsmax(sPass));
	get_cvar_string("sklepsms_db", sDB, charsmax(sDB));
...
public ss_service_data(name[], flags[])
	copy(sFlags, sizeof sFlags, flags);

public ss_service_addingtolist(id) 
{
	if(get_user_flags(id) & read_flags(sFlags) == read_flags(sFlags) || !bAdmin[id])
